...
def data_cleaning(df):
    format = '%Y-%m-%d %H:%M:%S'
    df.drop(columns=['server', 'title', 'datetakenunknown', 'views','url_l'],\
    inplace=True)

    # Condition 1 & 2: datetaken and dateupload are NOT identical & datetaken
    # time is valid
    for i in df.index:
        if df.datetaken[i] == df.dateupload[i] or \
                datetime.strptime(df.datetaken[i], format) > datetime.now():
            df = df.drop(index=i)
    ...
    for owner_list_iter in owner_list:
        ...
        analysis_cluster = analysis_cluster.append({'owner': owner_list_iter,
                                                    ...
                                                    'var_loc': var_loc},
                                                   ignore_index=True)
    # Condition 3: Retain data from tourist and remove users who upload
    # photos on a high percentage of working days, who upload photos with
    # a small geographic range or who upload photos more than 30 days apart
    # in a year
    for index, item_iter in analysis_cluster.iterrows():
        year_delete = []
        weekend_percent = item_iter.weekend / item_iter.total
        if weekend_percent < 0.5 or item_iter.var_loc < 0.1:
